!-------------------------------------------------------------------------------
! Method 4: Laplace evaluation for Model C
! Computes Laplacians for both concentration and phase field
!
...
contains
    module procedure laplace_evaluation
        ...
        ! First loop: Compute Laplacian of concentration and chemical potential
        do i = 1, this%Nx
            do j = 1, this%Ny
                ip = i + 1
                im = i - 1
                ...
                ! Chemical potential: mu = df/dc - kappa_c*laplacian(c)
                this%dummy_con(i, j) = this%dfdcon(i, j) - &
                    this%grad_coef_con * this%lap_con(i, j)
            end do
        end do
        
        ! Second loop: Compute Laplacian of chemical potential and phase field
        do i = 1, this%Nx
            do j = 1, this%Ny
                ip = i + 1
                im = i - 1
                ...
                this%phi_dummy(i, j) = this%dfdphi(i, j) - &
                    this%grad_coef_phi * this%lap_phi(i, j)
            end do
        end do
